'''Source for the standalone loader shipped inside an export bundle.

Kept as a template string rather than a file that gets copied, because the thing
being shipped must NOT import ``csllm`` — the whole promise of a deployment
package is that it works without this repo. Keeping it here, beside the exporter
that emits it, is what stops it from quietly drifting from the bundle format.

``tests/test_export.py`` executes the emitted file and asserts it encodes
identically to the real tokenizer, so a divergence fails the suite rather than
shipping.
'''

from __future__ import annotations

__all__ = ["LOADER_SOURCE", "README_TEMPLATE", "REQUIREMENTS"]
...
LOADER_SOURCE = '''"""Standalone loader for a CSLLM export bundle.
...
README_TEMPLATE = """\
# {name} — CSLLM export bundle

{num_params:,} parameters · {n_layer} layers × {n_head} heads × {n_embd} d_model ·
context {block_size} · vocab {vocab_size}

Exported {exported_at} by CSLLM engine {engine_version}.
...
- **Normalisation** — RMSNorm, pre-norm residuals (`norm_eps = {norm_eps}`)
- **Positions** — RoPE, **interleaved** channel pairs (`rope_theta = {rope_theta}`).
...
- **Attention** — causal multi-head, `head_dim = n_embd / n_head = {head_dim}` (even, as RoPE
